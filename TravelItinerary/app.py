...
try:
    ...
    train_fare_data = {cls: pd.read_csv(f"{DATA_FOLDER}/train_fare_matrix_{cls}.csv", index_col=0) for cls in train_classes}
...
df['Sightseeing Places Covered'] = df['Sightseeing Places Covered'].astype(str).apply(lambda x: [place.strip() for place in x.split('|') if place.strip().lower() != 'nan' and 'bus' not in place.lower()])
